...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Validatie
    const errors = [];
    if (!name || name.length < 2) errors.push('Naam is verplicht (minimaal 2 tekens)');
    if (!email) {
      errors.push('Email is verplicht');
    } else if (!isValidEmail(email)) {
      errors.push('Ongeldig emailadres formaat');
    }
    if (!subject || subject.length < 3) errors.push('Onderwerp is verplicht (minimaal 3 tekens)');
    if (!message || message.length < 10) errors.push('Bericht is verplicht (minimaal 10 tekens)');
...
${message}`,
    ...
    // Verstuur hoofdmail met timeout
    let mainEmailResult;
    try {
